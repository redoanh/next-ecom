...
import { useMenusQuery } from "@/redux/api/securityApi/menusApi";
...
const MenuAdmin = () => {
  ...
  // get department data
  const { data: transactionData, isLoading } = useMenusQuery({
    ...query,
    sortField: sortBy,
    ...
    },
    {
    ...
    },
     {
      ...
      render: (record: any) => (
        ...
        >
          <Link href={`/super_admin/security/menus/view/${record}`}>
            <Button
              style={{
                fontSize: "13px",
          ...
          <Link href={`/super_admin/security/menus/edit/${record}`}>
            <Button
              className="bg-[#FF5100]"
              style={{
                fontSize: "13px",
  ...
  return (
    <>
      ...
      >
        <UMBreadCrumb pageName="Menu" />

        <div>
          <ActionBar>
          <DarazCommonAddButton
              to={`/super_admin/security/menus/create`}
            >
              + Add New
            </DarazCommonAddButton>
